...
tutorial_data_path = mvpa2.cfg.get('location', 'tutorial data', default=op.curdir)

def get_raw_haxby2001_data(path=tutorial_data_path, roi='vt'):
    if roi is 0:
        # this means something special in the searchlight tutorial
        maskpath = op.join(path, 'haxby2001', 'sub001', 'masks', 'orig')
        nimg = nb.load(op.join(maskpath, 'hoc.nii.gz'))
        nimg_brain = nb.load(op.join(maskpath, 'brain.nii.gz'))
