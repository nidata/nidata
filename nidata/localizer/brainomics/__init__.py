...
class BrainomicsDataset(HttpDataset):
    ...
    def fetch(self, contrasts=None, n_subjects=None, get_tmaps=False,
        ...
        contrasts_wrapped = []
        # get a unique ID for each contrast. It is used to give a unique name
        # to each download file and avoid name collisions.
        contrasts_indices = []
        ...
        # It's better to perform several small requests than a big one because:
        # - Brainomics server has no cache (can lead to timeout while the
        #   archive is generated on the remote server)
        # - Local (cached) version of the files can be checked per contrast
        ...
        if url is None:
            url_csv = ("%sdataset/cubicwebexport.csv?rql=%s&vid=csvexport"
                       % (root_url, _urllib.parse.quote("Any X WHERE X "
                                                        "is Subject")))
            url_csv2 = ("%sdataset/cubicwebexport2.csv?rql=%s&vid=csvexport"
                        % (root_url,
                           _urllib.parse.quote("Any X,XI,XD WHERE X is "
                                               "QuestionnaireRun, "
        ...
        files = self.fetcher.fetch(filenames, resume=resume, force=force,
                                   verbose=verbose)
        anats = None
        masks = None
        tmaps = None
