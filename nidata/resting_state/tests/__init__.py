...
def test_nyu_dataset():
    ...
    assert_raises(NotImplementedError, NyuRestDataset().fetch, n_subjects=1, verbose=0)


def test_adhd_dataset():
    # Turns out this errors; so ... expect the error until we fix it!
    assert_raises(NotImplementedError, AdhdRestDataset().fetch, n_subjects=1, verbose=0)
...
def test_abide_dataset():
    ...
    dataset = AbidePcpDataset().fetch(n_subjects=1, verbose=0)
